...
function Checklist() {
  ...
  const canEdit = access.can('tasks.edit') || access.can('tasks.manage');
  ...
  return <>
    <DashboardCard title="Чек лист" eyebrow="Контроль исполнения" action={canCreate ? <Button size="sm" onClick={() => setCreateOpen(true)} disabled={loading}>+ Создать</Button> : null} className="dashboard-checklist" motion="left">
      {loading && !snapshot ? <DashboardWidgetState type="loading" /> : error && !snapshot ? <DashboardWidgetState type="error" text={error} onRetry={load} /> : !tasks.length ? <DashboardWidgetState title="Чек-лист пуст" text={canCreate ? 'Создайте первую задачу — она попадёт и в общий раздел задач, без отдельной копии данных.' : 'Задач пока нет. Новые задачи появятся здесь автоматически после их создания участником с правом управления.'} /> : <>
        ...
        <div className="dashboard-checklist__table" role="table" aria-label="Чек лист задач"><div className="dashboard-checklist__row dashboard-checklist__row--head" role="row"><span>Задача</span><span>Приоритет</span><span>Кто</span><span>Дата</span></div>{visibleTasks.map((task)=><div className={`dashboard-checklist__row ${task.done?'is-done':''} ${!canEdit?'is-read-only':''}`} role="row" key={task.id}><button className="dashboard-checklist__task" type="button" onClick={()=>toggleTask(task.id)} aria-pressed={task.done} aria-disabled={!canEdit} disabled={!canEdit} title={canEdit ? (task.description||'') : 'Только просмотр'}><span className={`dashboard-checklist__checkbox ${task.done?'is-checked':''}`}>{task.done?'✓':''}</span><span>{task.title}</span></button><span><Badge tone={task.tone}>{task.priorityLabel}</Badge></span><span className="dashboard-checklist__muted">{task.owner}</span><span className="dashboard-checklist__muted">{task.date}</span></div>)}{!visibleTasks.length?<div className="dashboard-checklist__empty">В этой категории пока нет задач</div>:null}</div>
      </>}
    </DashboardCard>
    {canCreate ? <ChecklistCreateModal open={createOpen} onClose={()=>setCreateOpen(false)} onCreate={createChecklistTask}/> : null}
  </>;
}
