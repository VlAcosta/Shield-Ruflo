...
function calculateRatingGrowth(values) {
  const measured = values.map(Number).filter((value) => Number.isFinite(value) && value > 0);
  if (measured.length < 2) return 0;
  const first = measured[0];
  const last = measured[measured.length - 1];
  return Number((((last - first) / Math.max(.01, first)) * 100).toFixed(1));
}

function deriveWeek(month, reviewMonth) {
  const labels = Array.isArray(month?.labels) ? month.labels.slice(-7) : [];
  const values = Array.isArray(month?.values) ? month.values.map(Number).slice(-7) : [];
  const measured = values.filter((value) => Number.isFinite(value) && value > 0);
  const received = Array.isArray(reviewMonth?.received) ? reviewMonth.received.map(Number).slice(-7) : [];
  return {
    labels,
    values,
    current: measured.length ? measured[measured.length - 1] : 0,
    growth: calculateRatingGrowth(values),
    reviews: received.reduce((sum, value) => sum + (Number.isFinite(value) ? value : 0), 0),
    positive: null,
    answered: null,
    healthScope: 'unmeasured',
  };
}
...
function Rating(){
  ...
  const { data:dashboardData,status,refresh }=useDashboardData();
  const section=dashboardData?.rating;
  const data=useMemo(()=>{
    if(section?.[period])return section[period];
    if(period==='week'&&section?.month)return deriveWeek(section.month,dashboardData?.reviews?.month);
    return null;
  },[dashboardData?.reviews?.month,period,section]);
  const rawLabels=useMemo(()=>Array.isArray(data?.labels)?data.labels:[],[data?.labels]);
  const rawValues=useMemo(()=>Array.isArray(data?.values)?data.values.map(Number):[],[data?.values]);
  const measuredSeries=useMemo(()=>rawValues.reduce((items,value,index)=>{
    if(Number.isFinite(value)&&value>0)items.push({value,label:rawLabels[index]||''});
    return items;
  },[]),[rawLabels,rawValues]);
  const values=useMemo(()=>measuredSeries.map((item)=>item.value),[measuredSeries]);
  const labels=useMemo(()=>measuredSeries.map((item)=>item.label),[measuredSeries]);
  const current=Number(data?.current)>0?Number(data.current):(values[values.length-1]||0);
  const growth=Number.isFinite(Number(data?.growth))?Number(data.growth):calculateRatingGrowth(values);
  const hasData=current>0&&values.length>0;
  const points=useMemo(()=>buildPoints(values),[values]);
  const path=useMemo(()=>buildPath(points),[points]);
  const scorePercent=Math.round((current/5)*100);
  ...
  const positiveMeasured=data?.positive!==null&&data?.positive!==undefined&&Number.isFinite(Number(data.positive));
  const answeredMeasured=data?.answered!==null&&data?.answered!==undefined&&Number.isFinite(Number(data.answered));
  const positive=positiveMeasured?Math.max(0,Math.min(100,Number(data.positive))):0;
  const answered=answeredMeasured?Math.max(0,Math.min(100,Number(data.answered))):0;
