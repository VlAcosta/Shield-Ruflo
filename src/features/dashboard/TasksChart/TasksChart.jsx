...
function TasksChart() {
  ...
  return (
    <DashboardCard title="Задачи" action={<div className="tasks-chart__header-actions"><PeriodMenu value={period} options={PERIOD_OPTIONS} onChange={handlePeriodChange} ariaLabel="Период статистики задач" /><span className="tasks-chart__head-icon"><ChartIcon /></span></div>} className="tasks-chart" motion="right">
      {status === 'loading' && !section ? <DashboardWidgetState type="loading" /> : status === 'error' && !section ? <DashboardWidgetState type="error" onRetry={refresh} /> : !hasData ? <DashboardWidgetState title="Задач пока нет" text="Создайте первую задачу — здесь появится распределение по направлениям, прогресс и просрочки." actionLabel="Открыть задачи" onAction={() => navigate('/tasks')} /> : <>
