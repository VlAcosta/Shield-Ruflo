...
function normalizeDashboardIntegration(value = {}) {
  const provider = String(value.provider || value.id || '').trim().toLowerCase();
  const meta = INTEGRATION_BY_ID[provider] || INTEGRATION_BY_ID[value.id] || {};
  const id = String(value.id || provider || value.name || '').trim();
  if (!id) return null;
  const status = String(value.status || 'disconnected').trim().toLowerCase();
  return {
    ...meta,
    ...value,
    id,
    provider,
    name: value.name || meta.name || provider || id,
    shortName: value.shortName || meta.shortName || meta.name || value.name || provider || id,
    category: value.category || meta.category || 'Provider API',
    tone: value.tone || meta.tone || 'violet',
    enabled: value.enabled !== false,
    status,
    link: value.link || '',
    lastSyncAt: value.lastSyncAt || value.lastSyncedAt || null,
    lastError: value.lastError || value.lastErrorCode || '',
  };
}
...
function Integrations() {
  ...
  const cachedIntegrations = useConnectedIntegrations();
  const { section, status, refresh } = useDashboardData('integrations');
  const canManage = access.can('integrations.manage');
  const integrations = useMemo(() => {
    const source = Array.isArray(section) ? section : cachedIntegrations;
    return source.map(normalizeDashboardIntegration).filter(Boolean).filter((item) => item.enabled !== false);
  }, [cachedIntegrations, section]);
  ...
  return <DashboardCard title="Интеграции" eyebrow="Provider hub" className="dashboard-integrations" motion="scale" action={<button type="button" className="dashboard-integrations__settings" onClick={openSettings}><SettingsIcon /><span>{canManage ? 'Настройки' : 'Открыть'}</span></button>}>
    {status === 'loading' && !Array.isArray(section) && !cachedIntegrations.length ? <DashboardWidgetState type="loading" compact />
      : status === 'error' && !Array.isArray(section) && !cachedIntegrations.length ? <DashboardWidgetState type="error" onRetry={refresh} compact />
        : integrations.length ? <>
