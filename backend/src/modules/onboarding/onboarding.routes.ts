...
export const onboardingRoutes: FastifyPluginAsync = async (app) => {
  app.get('/onboarding/state', { preHandler: [app.authenticate, app.authorize('company.view')] }, async (request) => {
  ...
  app.post('/onboarding/start', { preHandler: [app.authenticate, app.authorize('company.edit')] }, async (request) => {
    if (!request.auth?.organizationId) {
      throw new AppError({ code: 'ORGANIZATION_CONTEXT_REQUIRED', message: 'Рабочее пространство не выбрано', statusCode: 409 });
    }
  ...
  app.patch('/onboarding/state', { preHandler: [app.authenticate, app.authorize('company.edit')] }, async (request) => {
    if (!request.auth?.organizationId) {
      throw new AppError({ code: 'ORGANIZATION_CONTEXT_REQUIRED', message: 'Рабочее пространство не выбрано', statusCode: 409 });
    }
  ...
  app.post('/onboarding/complete', { preHandler: [app.authenticate, app.authorize('company.edit')] }, async (request) => {
